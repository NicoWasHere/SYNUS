...
let requests = new Map(); // name -> { type, opts } for calls made so far THIS tick
...
function use(name, type, opts, defaultValue) {
  ...
  requests.set(name, { type, opts, value }); // value included so the UI widget can read it without a separate getter
  return value;
}

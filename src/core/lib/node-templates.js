...
export const NODE_TEMPLATES = {
  ...
  canvas: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let canvas = use(Canvas2D, 256, 256);",
    "    const { ctx } = canvas;",
  ...
  square: () => [
    ...
    "    let canvas = use(Canvas2D, width, height);",
  ...
  html: () => [
    ...
    "    const use = useInstances(state);",
    "    const { width, height } = screenSize();",
    "    let html = use(Html, width, height);",
  ...
  lag: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let held = use(Lag, 10).tick(inputs.src); // fires every 10 ticks",
    "    return { screen: held };",
    "  },",
    "},",
  ].join('\n'),
  ...
  delay: () => [
    ...
    "    let out = use(Delay).tick(inputs.src, ticks);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  composite: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let out = use(Composite).tick(inputs.a, inputs.b, 'over', 1);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  mask: () => [
    ...
    "      maskSrc.upload();",
    "    }",
    "    let out = use(Mask).tick(inputs.src, maskSrc, { mode: 'lightness' });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  chroma_key: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let out = use(ChromaKey).tick(inputs.src, {",
  ...
  melt: () => [
    ...
    "      sections: 200,",
    "      jitter: 0.01,",
    "      t, // seed defaults to t - the per-section pattern keeps subtly reshuffling",
    "      thickness: 0.004,",
    "      drip: 0.01,",
    "      dieOff: 0.99,",
  ...
  transition: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let out = use(Transition).tick(inputs.src, {",
    "      trigger: newPatch,",
  ...
  gradient_map: () => [
    ...
    "      ramp.upload();",
    "    }",
    "    let out = use(GradientMap).tick(inputs.src, ramp, { channel: 'lightness' });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  kaleidoscope: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let out = use(Kaleidoscope).tick(inputs.src, 6);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  image: () => [
    ...
    "    const use = useInstances(state);",
    "    const { width, height } = screenSize();",
    "    let img = use(ImageSource, width, height);",
  ...
  video: () => [
    ...
    "    const use = useInstances(state);",
    "    const { width, height } = screenSize();",
    "    let vid = use(VideoSource, width, height);",
  ...
  webcam: () => [
    ...
    "    const use = useInstances(state);",
    "    const { width, height } = screenSize();",
    "    let cam = use(WebcamSource, width, height);",
    "    cam.tick();",
    "    return { screen: cam };",
    "  },",
    "},",
  ...
  hydra: () => [
    ...
    "    const use = useInstances(state);",
    "    const { width, height } = screenSize();",
    "    let h = use(HydraSource, width, height);",
  ...
  ramp: () => [
    ...
    "    const use = useInstances(state);",
    "    const ramp = use(Ramp);",
    "    let out = ramp.tick({ angle: 0, from: [0, 0, 0], to: [1, 1, 1] });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  gradient: (count = 2) => {
    ...
    lines.push(`    let out = use(Gradient).tick([${names}]);`, "    preview(out);", "    return { screen: out };", "  },", "},");
    return lines.join('\n');
  },
  ...
  noise: () => [
    ...
    "    let out = noise.tick({ scale: 4, z: t * 0.2, octaves: 4, type: 'perlin' });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  warp: () => [
    ...
    "  code(inputs, state) {",
    "    const use = useInstances(state);",
    "    let out = use(Warp).tick(inputs.src, [{ x: 0.5, y: 0.5, radius: 0.4, amount: 0.8 }]);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  ripple: () => [
    ...
    "  code(inputs, state, t) {",
    "    const use = useInstances(state);",
    "    let out = use(Ripple).tick(inputs.src, [{ x: 0.5, y: 0.5, frequency: 40, amplitude: 0.02, speed: 4 }], t);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  node_function: () => [
    ...
    "      return out;",
    "    });",
    "    let out = state.rotateThenShrink(inputs.src, t * 20);",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  sine: () => [
    ...
    "    let out = use(Brightness).tick(inputs.src, v * 0.6 - 0.3); // -0.3..0.3",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  instance: () => [
    ...
    "    let out = use(Instance).tick(inputs.src, count, (i) => ({",
    "      x: xs[i],",
    "      y: 0.5,",
    "      scale: sizes[i],",
    "      rotation: t * 40,",
  ...
  particle2d: () => [
    ...
    "  in: { src: 'other.screen' },",
    "  code(inputs, state, t) {",
    "    let out = particle2d(inputs.src, 24, 24, dot(), { t, min: 0.15, shakeSpeed: 1 });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  ascii: () => [
    ...
    "  in: { src: 'other.screen' },",
    "  code(inputs, state, t) {",
    "    let out = ascii2d(inputs.src, 60, 40, { fontSize: 16 });",
    "    return { screen: out };",
    "  },",
    "},",
  ].join('\n'),
  ...
  audio: () => [
    ...
    "    let mid = audio.band(250, 4000);",
    "    let treble = audio.band(4000, 12000);",
    "",
    ...
    "    preview(bass);",
    "",
    "    // bundled into one `audio` output - wire other.audio.bass (etc)",
    "    // instead of three separate top-level outputs",
    "    return { audio: { bass, mid, treble } };",
    "  },",
    "},",
  ].join('\n'),
  ...
  bpm: () => [
    ...
    "    const cc = 20;",
    "    let bpm = midiKnob(cc, { min: 60, max: 200, default: 120 });",
